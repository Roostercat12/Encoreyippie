...
/obj/structure/fake_machine/mail/proc/handle_merctoken(obj/item/merctoken/token, mob/user)
	...
	switch(H.merctype)
		...
		if(1) new /obj/item/clothing/neck/mercmedal/qadirid(drop_location)
		if(2) new /obj/item/clothing/neck/mercmedal/sterkenstadten(drop_location)
...
/obj/structure/fake_machine/mail/proc/display_marquette(mob/user)
	...
	if(HAS_TRAIT(user, TRAIT_PURITAN))
		contents += "  <a href='byond://?src=[REF(src)];locktoggle=1]'> INQUISITOR'S LOCK: [inqonly ? "YES":"NO"]</a>  <BR>"
	else
		contents += "  INQUISITOR'S LOCK: [inqonly ? "YES":"NO"]  <BR>"
	contents += "<a href='byond://?src=[REF(src)];eject=1'>DENARIUS LOADED: [inqcoins]</a><BR>"
