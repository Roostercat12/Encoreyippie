...
/obj/item/clothing/head/headband/naledi
	...
	armor_type = /datum/armor/head/angros_hood //Higher-tier protection for pugilist-centric classes. Fits the 'glass cannon' style, and prevents instant death through a glancing headshot on the intended archetype.
	blade_dulling = DULLING_BASHCHOP
	body_parts_covered = HEAD|HAIR|EARS
	max_integrity = ARMOR_INT_SIDE_STEEL //High leather-tier protection and critical resistances, steel-tier integrity. Integrity boost encourages hand-to-hand parrying. Weaker than the Unsundered Thorns.
...
/obj/item/clothing/head/roguehood/angros
	...
	armor_type = /datum/armor/head/angros_hood
	dynamic_hair_suffix = ""
	edelay_type = 1
	adjustable = CAN_CADJUST
	toggle_icon_state = TRUE
...
/obj/item/clothing/head/roguehood/angros/AdjustClothes(mob/living/carbon/user)
	if(loc == user)
		if(adjustable == CAN_CADJUST)
			adjustable = CADJUSTED
			if(toggle_icon_state)
...
/obj/item/clothing/head/roguehood/angros/ResetAdjust(mob/user)
	. = ..()
	if(toggle_icon_state)
		icon_state = "[initial(icon_state)]_t"
...
/obj/item/clothing/head/roguehood/angros/confessor
