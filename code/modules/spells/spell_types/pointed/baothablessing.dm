/datum/action/cooldown/spell/hertanneablessings
	name = "Hertannea's Blessings"
...
/datum/action/cooldown/spell/hertanneablessings/is_valid_target(atom/cast_on)
	. = ..()
	if(!.)
		return FALSE
...
/*	if(istype(cast_on.patron, /datum/patron/angros))
		...
		return
*/
	if(cast_on.has_status_effect(/datum/status_effect/buff/druqks/hertannea))
		to_chat(owner, span_warning("They're already blessed by these effects!"))
		return
	cast_on.apply_status_effect(/datum/status_effect/buff/druqks/hertannea) //Gets the trait temorarily, basically will just stop any active/upcoming ODs.
